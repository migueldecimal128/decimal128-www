...
26411.119 review edition (SDK 11.0.100-rc.1.26411.119).
...
Cohort is UNCHANGED from the 26401.101 edition: System.Decimal stays retired and the
...
Store rows behind this edition: arm64 Rcsbid10 + x86_64 xRcsbid10, both engine 64e891a
under SDK 11.0.100-rc.1.26411.119 — the third consecutive edition-pair measured on one
SDK across both machines.

ONE CHANGE FROM THE 26401.101 GENERATOR — the x86 libbid source. That edition resolved
libbid on x86_64 to lang c-rc1 (run xRcrc1, the 26380.103-session C arm) and had to warn
in section 3 that those rows were cross-session against the SUT. The i9 has since
re-measured all nine ports in ONE session (store commit 2620739), so lang c on x86_64 now
carries same-session C numbers; that sweep verified libbid from it against c-rc1 at
geomean 1.003 / median 1.004, i.e. the two agree to well under a percent. lang c-rc1 is
therefore redundant and the priority list collapses to ("c",) on both arches. The rows are
still cross-session against xRcsbid10 (the C sweep and the csharp-bid sweep are a week
apart on a box with day-scale drift), so section 3 keeps a narrowed caveat rather than
dropping it.

Usage (run from this directory):
    gen_bench_net11_rc1_26411_119.py --emit  <id>        print one table block to stdout
    gen_bench_net11_rc1_26411_119.py --splice [FILE]     splice all blocks into FILE in place
    gen_bench_net11_rc1_26411_119.py --check  [FILE]     diff each block vs the block in FILE
FILE defaults to the review page (../reviews/dotnet/net11-rc1-26411.119.md).
"""
import json, glob, os, re, sys

HERE   = os.path.dirname(os.path.abspath(__file__))
TARGET = os.path.normpath(os.path.join(HERE, "..", "reviews", "dotnet", "net11-rc1-26411.119.md"))
...
# libbid resolves to the standing cross-port C arm (lang c) on BOTH arches now — see the
# module docstring: the i9's one-session nine-port re-sweep made c and c-rc1 agree to 0.4%
# on x86_64, so the c-rc1 special case has no purpose left. Order is editorial: SUT first.

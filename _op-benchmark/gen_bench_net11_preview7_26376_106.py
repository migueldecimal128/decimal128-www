#!/usr/bin/env python3
"""Four-implementation benchmark tables for the .NET 11 preview 7 DAILY BUILD
26376.106 review edition (SDK 11.0.100-preview.7.26376.106).
...
gen_bench.py reads — and splices four-column absolute-ns tables into the review page's marker
regions:  <!-- BEGIN GENERATED <id> --> ... <!-- END GENERATED <id> -->
(All prose in the page stays hand-written OUTSIDE the markers.)

Columns (editorial order — system-under-test first):
    Decimal128 (.NET 11)  |  libbid  |  decimal128-csharp  |  System.Decimal
A cell is the headline ns/op for that impl at that band/arch, or an em dash where the impl
has no record (System.Decimal on a band its 28-digit range can't represent — and on FMA,
which Decimal128 DOES implement as of SDK 26376.106, unlike at the prior edition).

Usage (run from this directory):
    gen_bench_net11_preview7_26376_106.py --emit  <id>       print one table block to stdout
    gen_bench_net11_preview7_26376_106.py --splice [FILE]     splice all blocks into FILE in place
    gen_bench_net11_preview7_26376_106.py --check  [FILE]     diff each block vs the block in FILE
FILE defaults to the review page (../reviews/dotnet/net11-preview7-26376.106.md).
"""
import json, glob, os, re, sys

HERE   = os.path.dirname(os.path.abspath(__file__))
TARGET = os.path.normpath(os.path.join(HERE, "..", "reviews", "dotnet", "net11-preview7-26376.106.md"))

# Columns: (lang, impl, header). `lang` selects which results.<lang>.jsonl holds the record
# (libbid is C-hosted; the rest are the csharp arm). Order is editorial: SUT first.
COLS = [("csharp", "System.Numerics.Decimal128", "Decimal128 (.NET 11)"),
        ("c",      "libbid",                      "libbid C"),
        ("csharp", "d128",                        "decimal128-csharp"),
        ("csharp", "System.Decimal",              "System.Decimal")]

# base marker id -> (profile, [(op, cat), ...]). Each base renders for both arches; the x86
# variant uses the "<id>-x86" marker, rendered directly below its arm64 twin in §7.
TABLES = {
  ...
  # mirrored on sub). NOT comparable to the 26366.102 edition's blended add/sub rows.

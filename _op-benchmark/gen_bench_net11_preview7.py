#!/usr/bin/env python3
"""Four-implementation benchmark tables for the .NET 11 *preview 7* review.
...
has no record (System.Decimal on a band its 28-digit range can't represent; Decimal128 on
FMA, which it does not implement).

Usage (run from this directory):
    gen_bench_net11_preview7.py --emit  <id>       print one table block to stdout
    gen_bench_net11_preview7.py --splice [FILE]     splice all blocks into FILE in place
    gen_bench_net11_preview7.py --check  [FILE]     diff each block vs the block in FILE
FILE defaults to the review page (../reviews/dotnet/net11-preview7.md).
"""
import json, glob, os, re, sys

HERE   = os.path.dirname(os.path.abspath(__file__))
TARGET = os.path.normpath(os.path.join(HERE, "..", "reviews", "dotnet", "net11-preview7.md"))
...
# variant uses the "<id>-x86" marker (matching the review page's Appendix B).
TABLES = {
  "net11-pfin-abs": ("P-fin", [("add","MIX"),("sub","MIX"),("mul","CP"),("mul","WP"),
                               ("div","CD"),("div","WD"),("div","ET"),("div","PT")]),
  "net11-add-abs":  ("P-gen", [("add", c) for c in ("SQ","NQ","MQ","OQ","FQ")]),
  "net11-sub-abs":  ("P-gen", [("sub", c) for c in ("SQ","NQ","MQ","OQ","FQ")]),

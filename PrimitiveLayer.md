...
| D18 | **`Stripped128StripCount` settles the strip result shape**: a typed aggregate (`stripped128`, `stripCount`), value-returned, resolving the open typed-aggregate-vs-mutate-in-place question in favor of family consistency. At three dwords it is the smallest aggregate in the family — if escape analysis cannot scalarize it, nothing value-returns on the JVM, and P5's measured fill-in fallback exists for exactly that contingency. The tuple it replaces was the last in the layer's orbit. The function's full-form D9 spelling and its relocation (the Swift body still calls core's 128-bit Barrett pow10 unit) remain open. |

...
| strip | `u128StripTrailingZeroDigits(x, maxToStrip)` ◆ | `Stripped128StripCount` | settled primitive; result shape settled (D18); full-form spelling still pending |
...
| `Stripped128StripCount` | stripped128, stripCount | `u128StripTrailingZeroDigits` (D18) |
...
- **strip relocation and full-form name** — the result shape is settled (D18, `Stripped128StripCount`), but the Swift body still calls core's 128-bit Barrett pow10 unit, so the settled-primitive classification is not yet physical; relocation waits on classifying that unit (or re-expressing strip), and the D9 full-form spelling follows.
